        ...
        <div class="col-md-6 col-12 carousel-wrapper">
          <swiper :options="swiperOption" ref="mySwiper">
            <swiper-slide v-for="(imagePath) in selectedProject.images" :key="imagePath">
              ...
              <video :src="imagePath" controls class="img-fluid" loading="lazy" v-if="imagePath.endsWith('mp4')"></video>
              <img v-else class="img-fluid" loading="lazy" :src="imagePath"/>
            </swiper-slide>
                ...
                projects: {
                    ...
                    'smartcare': {
                        ...
                        title:
                            'SmartCare',
                        url: null,
                        teaser: 'How can elderly people profit from a connected and smart world? The goal of the Internet of Things course was to create smart solutions to support disabled and/or handicapped people in their daily lifes.',
                        description: 'SmartCare is the name of the prototype, which was developed during the IoT course. We noticed that for elderly people moving around in ther own home can be very hard and exhausting, especially when they are handicapped. So even basic tasks like switching off the radio, ligths or the TV can be a big challenge for them. For that reason we leveraged Smart Home devices like Philips Hue lights or Sonos speakers which can be controlled by simple buttons. This "button station" can be placed anywhere, e.g. close to the handicapped person\'s bed. Furthermore we used a Microsoft Band (wearable device) to automatically switch off all devices whenever we detect that the person is sleeping. Last but not least we created a mobile app to control all the devices from remote and also see the vital parameters recorded by the Microsoft Band, so that for example a caregiver is always informed about the person\'s health status.',
                        images: [
                    ...
                    'smartchart': {
                        ...
                        title:
                            'SmartChart',
                        url: null,
                        teaser: 'What will be the next Number 1 Hit? How successful will a new song be? In my bachelor thesis I tried to predict chart positions of songs using neural networks.',
                        description: 'Being able to predict how successful a certain song will be is not only of interest for the artist himself, but also for the producers, labels, chart ranking agencies, advertisers etc... So I asked myself, whether it is possible to predict a songs chart position based on two kinds of features: On the one hand I collected a large number of metadata including information about the artist (e.g. how often was this artist present in the top 100 charts) and former songs of the artist (e.g. how long were the songs in the top 100 charts). On the other hand I did an in-depth analysis of the songs on the signal level (both time domain and frequency domain) (such as Auto Correlation, MFCC (Mel-Frequency Cepstral Coefficients), Spectral Centroid and many more). After collecting all the information for a comprehensive data set I trained a neural network to make a prediction. Instead of predicting the exact chart position (which is hardly possible) by a regression algorithm, I used classification to predict the \'chart position range\' (i.e. position 1, position 2-5, position 6-20, position 21-50, position 51-100 and not in the charts). The results were quite promising, but due to an highly unbalanced dataset (sparse data for class 1 (chart position 1), dense data for class 6 (not in the charts)), the algorithm was biased.',
                        images: [
